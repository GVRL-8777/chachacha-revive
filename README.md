...
자료들을 기반으로 살려 보았습니다. **서버 없이 스마트폰 혼자 돌아갑니다.**

종료된 지 오래되었지만 그럼에도 계속 유지보수해 나갈 계획입니다.
**남은 자료를 가지고 계시다면 보내 주세요.** 무엇이 필요한지는 아래
[찾습니다](#찾습니다) 에 적어 두었습니다.

> **다함께 차차차 2** 는 남은 자료가 더 적어 복구가 훨씬 어렵습니다.
> 그쪽 자료도 함께 찾고 있습니다.
...
| **한 벌에 둘 다** | 로컬판과 서버판을 한 APK 에 넣고 **게임 안에서 전환** |
...
**넷입니다.** 하나는 뼈대이고, 둘은 한국어화 재료이고, 하나는 맵입니다.
...
**왜 하필 중국판인가.** 이 배포판만 **CDN 계층이 없습니다.** 자산이 전부
APK 안에 들어 있어서 이미 죽은 넷마블 CDN 을 안 찾습니다. 막히는 곳은
360/NetmarbleS 소셜 플러그인 초기화 하나뿐이고, 그건 건너뛸 수 있습니다.
...
이 판의 `tb_systemtext` 가 **한국어 원본**입니다. 게임 안의 거의 모든 글이
여기서 나옵니다.
...
| | 왜 |
|---|---|
| **Python 3.9 이상** | 도구 전부 (여기서는 3.13 으로 씁니다) |
...
| [`docs/LOCALAPK.md`](docs/LOCALAPK.md) | 서버 없이 돌리기 · 한 벌에 둘 다 넣기 |
...
- 문제 제보 시 가능한 한 상세한 설명을 첨부해 주시면 감사하겠습니다.
  ...
  좋습니다.
- **자료 제보는 Issue 로 부탁드립니다.** 남들 눈에 보여야 "나도 그거
  있는데" 가 이어집니다. 파일이 커서 올리기 어려우면 Issue 에 먼저
  남겨 주세요.