...
VOX 클립이 있는 곳은 세 군데입니다.
  ...
  · 중국판(x77)                    — 4명 80개, 중국어
  · survey/racechachachaforkakao   — 11명, **한국어** (카카오 배포판)
    차 이름이 Garuda·Hummie·Magnum 이라 한때 '동남아/영문판'으로 적어
    두었는데 **틀렸습니다**. 귀로 확인했습니다(2026-08-24) — 열한 명
    전부 한국어입니다.
...
## 그래서 무엇을 고를 것인가
  ...
  `--kr`     기본 드라이버 4명을 **초기판(8.apk)** 것으로
  (기본값)    11명을 **카카오판**으로 통일

둘 다 한국어입니다. 고르는 것은 언어가 아니라 **어느 판의 녹음인가**입니다.

`--kr` 을 쓰면 기본 4명은 초기판 녹음, 나머지 7명은 카카오판 녹음이
됩니다. **말은 안 섞입니다** — 둘 다 한국어입니다.

이름이 1:1 로 맞으므로 짝짓기는 이름으로 합니다. 소리 길이가 달라 제자리
덮어쓰기는 못 하고 파일을 통째로 다시 씁니다(다행히 보이스는 파일 하나에
클립 하나씩 들어 있습니다). 원본은 backup/ 에 남습니다.

  python voicefix.py            카카오판 것으로 맞춥니다 (11명 통일)
...
def main():
    ...
    if not os.path.isdir(os.path.join(DONOR, A.DATA)):
        raise SystemExit(
            ...
            % DONOR)
    print('공급원: %s (%s · 둘 다 한국어)'
          % (which, '초기판 8.apk' if which == 'kr' else '카카오판'))
