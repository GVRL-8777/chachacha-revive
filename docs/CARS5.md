...
### 드라이버 카드가 기본 4인방 말고는 소리가 안 나던 것 (`tools/drvvoice.py`)

카드마다 소리 자리가 둘 있다 — `UIButtonSound`(누를 때 = CHOICE) ·
`AudioSource`(장착될 때 = EQUIP). 둘 다 스크립트가 클립을 안 고르고
프리팹에 물려 있는 것을 그대로 낸다. 그런데 열두 칸 중 넷(도캉·사라·빈·나유비)만
물려 있었다 — 중국판 APK 안에는 이 넷의 VOX 80개뿐이고, 나머지 여섯 명의
목소리는 나중에 **번들**로 넣은 것이라 프리팹이 못 가리켰다(프리팹의 PPtr은
APK 안 자산만 가리킬 수 있다).

번들에 있던 클립 12개(갸루상·김준현·앵그리성호·정신이·나정비·안별이의
CHOICE·EQUIP)를 APK 트리로 옮겨 새 자산 파일 하나로 묶고, 빈 자리 열두 곳의
PPtr을 채웠다. 고치는 김에 카드 8~11(나중에 복제해 넣은 넷)의 버튼 클릭음
fileID가 범위 밖을 가리켜 망가져 있던 것도 같이 바로잡았다.

쌈바여인·한이 가희는 목소리가 **어느 판에도 없어** 계속 조용하다.

한 탭으로 선택=장착이 한 번에 되는 구조라(`DriverUnit::OnBuyDriver`),
CHOICE는 누르는 즉시·EQUIP은 서버 응답 직후 거의 붙어서 난다 — 이미 산
캐릭터를 고르면 두 목소리가 다 나되 뒤에 나오는 EQUIP이 더 뚜렷하게 들리고,
아직 안 산 캐릭터를 눌러 구매창이 뜰 때는 CHOICE만 단독으로 난다.
