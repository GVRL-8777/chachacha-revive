...
def make_manifest(name, entries, main=(0, 0), all_pids=None):
    """entries: [(컨테이너키, pathID)] — 번들에서 이름으로 꺼낼 자산 목록

    all_pids 를 주면 프리로드 표에 번들 안 오브젝트를 전부 싣고, 각 항목이
    그 전체를 가리키게 한다. 에디터가 만든 번들도 항목마다 필요한 오브젝트를
    모두 늘어놓는데, 뿌리 하나만 실으면 엔진이 자산을 못 만들어 내는 듯하다.
    """
    out = w_str(name)
    pre = list(all_pids) if all_pids else [pid for _, pid in entries]
    out += struct.pack('<i', len(pre))
    for pid in pre:
        out += w_pptr(0, pid)
    # m_Container
    # 엔진 판본에 따라 키를 그대로 찾기도 하고 소문자로 맞춰 찾기도 한다.
    # 둘 다 넣어 두면 어느 쪽이든 걸린다.
    rows = []
    for i, (key, pid) in enumerate(entries):
        seen = []
        for k in (key, key.lower(), key.split('/')[-1], key.split('/')[-1].lower()):
            if k and k not in seen:
                seen.append(k)
                rows.append((k, i, pid))
    out += struct.pack('<i', len(rows))
    for key, i, pid in rows:
        out += w_str(key)
...
def build(src_path, out_path, bundle_name, entries, main_pid=0):
    ...
    # 유니티는 번들에서 AssetBundle 오브젝트를 오브젝트 테이블 **앞쪽**에서 찾는 것으로 보인다.
    # pathID 는 그대로 두고 테이블 순서만 맨 앞으로 놓는다(내부 참조는 pathID 기준이라 안전하다).
